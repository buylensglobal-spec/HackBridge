...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (event) => {
      ...
      const cards = document.querySelectorAll('#hack-list [data-card-name]');
  ...
  const bindJoinButtons = () => {
    document.querySelectorAll('[data-join]').forEach((button) => {
      ...
      button.addEventListener('click', () => {
        ...
        window.location.href = `join-hackathon.html?title=${encodeURIComponent(title)}`;
      });
    });
  };

  if (document.getElementById('hack-list')) {
    const hackList = document.getElementById('hack-list');
    onValue(ref(database, 'hackathons'), (snapshot) => {
      const hackathons = snapshot.val() || {};
      const entries = Object.entries(hackathons).map(([key, value]) => ({
        key,
        ...value,
        hackathonName: value.hackathonName || key
      }));

      hackList.replaceChildren();
      if (!entries.length) {
        const emptyState = document.createElement('p');
        emptyState.className = 'muted project-empty-state';
        emptyState.textContent = 'No hackathons added yet.';
        hackList.append(emptyState);
        return;
      }
      ...
      entries.forEach((hackathon) => {
        ...
        article.dataset.cardName = hackathon.hackathonName;

        const title = document.createElement('h3');
        title.textContent = hackathon.hackathonName;

        const badgeRow = document.createElement('div');
        badgeRow.className = 'badge-row';
        const badgeOne = document.createElement('span');
        badgeOne.className = 'badge';
        badgeOne.textContent = hackathon.teamSize ? `Team ${hackathon.teamSize}` : 'Team size';
        const badgeTwo = document.createElement('span');
        badgeTwo.className = 'badge';
        badgeTwo.textContent = hackathon.organizationName || 'Hackathon';
        const badgeThree = document.createElement('span');
        badgeThree.className = 'badge';
        badgeThree.textContent = hackathon.endDate ? `Ends ${hackathon.endDate}` : 'Open now';
        badgeRow.append(badgeOne, badgeTwo, badgeThree);

        const description = document.createElement('p');
        description.textContent = `${hackathon.organizationName || 'Hackathon organizer'} is hosting this event for students and builders.`;

        const list = document.createElement('ul');
        list.className = 'list-check';
        const itemOne = document.createElement('li');
        itemOne.textContent = hackathon.organizationEmail ? `Contact: ${hackathon.organizationEmail}` : 'Student-friendly challenge';
        const itemTwo = document.createElement('li');
        itemTwo.textContent = hackathon.prize1 ? `Prize: ${hackathon.prize1}` : 'Portfolio-ready experience';
        const itemThree = document.createElement('li');
        itemThree.textContent = hackathon.teamSize ? `Team size: ${hackathon.teamSize} members` : 'Flexible team format';
        list.append(itemOne, itemTwo, itemThree);

        const metaRow = document.createElement('div');
        metaRow.className = 'meta-row';
        const status = document.createElement('span');
        status.textContent = hackathon.endDate ? `Ends ${hackathon.endDate}` : 'Open now';
        const button = document.createElement('button');
        button.className = 'action-btn primary-btn';
        button.type = 'button';
        button.dataset.join = 'true';
        button.textContent = 'Join';
        metaRow.append(status, button);

        article.append(title, badgeRow, description, list, metaRow);
        hackList.append(article);
      });
  ...
  if (signInForm) {
    signInForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        const accountRef = ref(database, `students/${generateStudentKey(existingStudents.val())}`);
...
function generateStudentKey(users) {
  const existingKeys = new Set(Object.keys(users || {}));
...
async function updateProfileChip(profileId) {
  ...
  try {
    ...
    renderStudentDetails(student, profileId);
...
function renderStudentDetails(student, profileId) {
